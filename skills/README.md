...
| `launch-agent`    | `/launch-agent <folder> [--name <name>] <prompt>` | Dispatches a background agent: creates the topic folder, saves the launch prompt in it, names the job, and tells the child to point its working files at that folder. A slug resolves at the repository root; an absolute path goes anywhere and skips the near-match guard. |
