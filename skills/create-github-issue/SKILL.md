...
Separately, write the collected title to its own one-line file in the same notes folder, with description `issue-title`, using the title the reader returned in Step 1 or the inline title. The title file keeps the user-authored text out of the create command line so Step 5 can pass it to gh verbatim.
