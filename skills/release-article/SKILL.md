...
3. Only when no correlated match exists, write the handoff draft via `/note` with description `release-article-handoff`, in the standardized draft shape that `/create-github-issue` reads: a front-matter block with a `target-repo: couimet/couimet.github.io` field, a `#` heading as title, and a body containing: the source repository URL and name; the release version, tag, or changelog section; the local article path (the real project path, or a statement that the draft is local and not yet in the repository when the article lives at the Step 2 `.claude-work/` fallback — `.claude-work/` paths never appear in the draft, whose body becomes a GitHub issue); the public article URL when published; the article status; relevant release, PR, and issue URLs; the proposed `projects` value with the reason for it; a reference to the promotion copy draft; and a checklist for updating `_data/promotions.yml` and reviewing the centralized publishing tone. The target-repo field routes the issue to the registration repo, and the `#` heading keeps the title separate from the body, so `/create-github-issue` does not need to guess.
