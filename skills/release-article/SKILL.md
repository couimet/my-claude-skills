...
Locate the changelog. Start at the repo root `CHANGELOG.md`. When `$ARGUMENTS` names a release, verify the root changelog contains a heading for that exact release, not merely that it has version headings. In a monorepo the root changelog is often only an index, so when the root has no version headings or does not contain the requested release, scan `packages/*/` for a package that pairs a `CHANGELOG.md` with a README containing a "Featured In" section. When several packages match, narrow the candidates against the requested release or tag when `$ARGUMENTS` names one, and always against the project identity: the package whose changelog contains the target release and whose README names the product being released. When `$ARGUMENTS` is empty, narrow by project identity and README context only. If multiple candidates still match, ask the user via `AskUserQuestion` which changelog to use before proceeding. If no package candidate matches, **STOP** and ask the user via `AskUserQuestion` which changelog to use. If the requested release exists in no changelog at all (root or packages), **STOP** and ask the user via `AskUserQuestion` for the correct source or release. Report which changelog was used.
...
- [ ] Changelog located (root or per-package) with the requested release verified in it, and target release identified
