...
4. **Lexicographic order equals creation order.** A byte-order sort of a directory lists the files from oldest to newest. To find the newest file that matches a pattern, take the maximum. Any caller that resolves "the most recent" file relies on this. Skip a zero-byte candidate while you do. Property 2 defines an empty file at a stamped path as a reservation whose caller has not written yet, and the newest file matching your pattern is exactly the one most likely to be one.

You never release a path you decide not to use. The script sweeps an abandoned reservation on a later call, so a caller that resolves a path and then writes nothing leaves nothing behind for long.
