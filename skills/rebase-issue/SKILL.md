...
2. **Find PR description in notes/** — searches for the most recent nonempty `*finish-issue-<ID>*` file in `<folder>/notes/`, skipping any zero-byte match, which is an unwritten reservation rather than a description
...
- [ ] Stacked mode (if applicable): apply-stacked-diff.sh completed successfully, no old-commit residue in staged changes
