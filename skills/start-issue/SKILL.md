...
**Grill the draft before creating the working document.** Draft the plan content in-session, then run `/g2q` on the draft. It grills the draft for genuinely open ambiguities (applying the trigger predicate at the top of `/g2q`, the single source of trigger truth), creates a questions file under `<base>/issues/<NUMBER>/questions/` when it finds any, and reports whether any were raised. The answer gates how the working document is created in 4a/4b:
