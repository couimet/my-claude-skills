...
Every commit message file must end with a footer line identifying the skill that generated it. Fill `<VERSION>` from:

```bash
~/.claude/skills/issue-context/skill-version.sh commit-msg
```
...
Format: plain text, no markdown. One continuous line per paragraph. Every message ends with a blank line followed by the skill footer as the very last line. This footer replaces the default `Co-Authored-By` — do not add both. Fill `<VERSION>` in the footer from `skill-version.sh commit-msg`.
...
5. **Run the prose checker.** Run `check-prose.sh` on the file you just wrote and fix every line it names. See `/prose-style` Rule 1.
   Also verify the complete skill footer contract: the footer is the very last line of the file, is separated from the preceding content by exactly one blank line, contains no `Co-Authored-By` trailer anywhere in the file, and the version matches what `skill-version.sh commit-msg` printed.
