...
Every commit message file must end with a git trailer line identifying the skill that generated it. The `Files:` footer comes first, then the `Skill:` trailer as the very last line. Read the `version:` field from this SKILL.md's front matter to fill `<VERSION>`.

```text
Skill: /commit-msg v<VERSION> • https://github.com/couimet/my-claude-skills
```

This `Skill:` trailer replaces the default `Co-Authored-By: Claude <noreply@anthropic.com>` line — do not add the `Co-Authored-By` trailer. The skill footer is the sole attribution.
...
Format: plain text, no markdown. One continuous line per paragraph. Every message ends with a `Files:` line derived from the effective diff's stat output, then a `Skill:` git trailer as the very last line. The `Skill:` trailer replaces the default `Co-Authored-By` — do not add both. Derive the `Files:` line by parsing each stat line into `path (+N, -N)`, omitting `-N` when zero, joining with commas, and skipping the summary line. Fill `<VERSION>` in the `Skill:` trailer from this SKILL.md's front matter `version:` field.
...
Also verify the `Skill:` trailer is present and the version matches this SKILL.md's front matter `version:` field.
