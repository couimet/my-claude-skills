...
Every commit message file must end with a footer line identifying the skill that generated it. The `Files:` footer comes first, then a blank line, then the skill footer as the very last line. Read the `version:` field from this SKILL.md's front matter to fill `<VERSION>`.

```text

Generated by /commit-msg v<VERSION> • [my-claude-skills](https://github.com/couimet/my-claude-skills)
```
...
Format: plain text, no markdown. One continuous line per paragraph. Every message ends with a `Files:` line derived from the effective diff's stat output, then a blank line, then a skill footer as the very last line. This footer replaces the default `Co-Authored-By` — do not add both. Derive the `Files:` line by parsing each stat line into `path (+N, -N)`, omitting `-N` when zero, joining with commas, and skipping the summary line. Fill `<VERSION>` in the footer from this SKILL.md's front matter `version:` field.
...
Also verify the skill footer is present and the version matches this SKILL.md's front matter `version:` field.
