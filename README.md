...
- [I sold you on /scratchpad. Then I migrated to /note.](https://dev.to/couimet/i-sold-you-on-scratchpad-then-i-migrated-to-note-4n1o) - DEV Community
...
- **Organized, not scattered.** Working files go under `.claude-work/` with type subdirectories (`scratchpads/`, `questions/`, `commit-msgs/`), timestamped filenames, and issue-scoped organization. No manual file management — and `/cleanup-issue` removes the issue's working directory when you're done.
...
| `/launch-agent <folder> [--name <name>] [--here] <prompt>`     | Dispatch a background agent with its topic folder created, its launch prompt saved inside it, and its display name set, all in one call — `--here` sets the same topic up in the session you are already in and starts no agent, so invoking it from a session you started for the purpose leaves one row in `claude agents` rather than two |
...
| `/set-work-folder <folder> [name]`                             | Point this session's working files at a folder instead of letting the current git branch decide — `--worktree` does the same for a checkout, and `--clear` falls back to that marker, then the branch                                                                                                                                        |
