...
**Stamp every working file `YYYYMMDD-HHMMSS-<slug>.<ext>` through one helper, and retire `auto-number.sh` along with the `NNNN-` scheme.**
...
A timestamp is a superset of an ordinal here. Lexicographic sort still equals creation order, so the sequential property survives and creation time comes along for free. It also removes the directory scan. A stamp needs no neighbours, so a malformed sibling cannot poison it. There is also no read-then-write window between choosing a name and writing it.
...
**Same-second collisions get a `-2`, `-3` disambiguator.** A scan could not collide. A stamp can, when two calls in one second share a slug and a directory. The script tries the plain name, then appends a disambiguator before the extension while the path exists. The timestamp stays truthful and only the colliding name carries the extra token. The alternative, advancing the second until the path is free, names a second the file was not created in.
