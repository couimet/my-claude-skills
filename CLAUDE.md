...
**Quote placeholder arguments in the commands you show:** Wherever a skill shows a command carrying a `<placeholder>` argument, fenced or inline in prose, quote that argument: `remove-issue-dir.sh "<folder>" --id "<ID>"`. A configured work folder is an arbitrary absolute path and may contain spaces, and the reader substituting the placeholder has no other signal that the result is one argument. Where the placeholder sits inside a string that is already double-quoted, quote it with single quotes so the pairs never nest: `--title "$(cat '<TITLE_FILE_PATH>')"`, never `--title "$(cat "<TITLE_FILE_PATH>")"`. Nesting parses correctly in bash and still reads as a typo, and a reader pasting a path into the inner pair has no way to see which quote closes which. Two things stay unquoted, and both follow from the rule rather than excusing it: a usage synopsis showing a script's argument grammar, as in `/issue-context`, because quoting grammar reads as part of the grammar; and an optional token that may expand to nothing, as in `resolve-target.sh "<NUMBER>" [$ARGUMENTS]`, because quoting it would pass an empty argument where the script expects none.
