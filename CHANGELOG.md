...
- Cleaning up a work item while a worktree folder is set removes the plan pointers and the breadcrumb, and leaves the notes and questions, which sit together for every work item rather than in per-item directories. `/cleanup-issue` now says this at its confirmation prompt, so a finished cleanup is not read as more than it was. ([issues/267](https://github.com/couimet/my-claude-skills/issues/267))
