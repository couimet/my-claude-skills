...
- `/start-issue` and `/tackle-pr-comment` grill their drafted plan or analysis via `/g2q` before creating the working document, and gate plan production on the answers: when the pass raises questions, the note or scratchpad is created only as a pending stub with a banner naming the questions file by full path, and the finalized plan is written after the answers arrive. ([issues/236](https://github.com/couimet/my-claude-skills/issues/236))
