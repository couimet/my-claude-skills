...
Entries are organized using [Keep a Changelog](https://keepachangelog.com/) categories: **Added**, **Changed**, **Deprecated**, **Removed**, **Fixed**, **Security**. Not every release uses every category — include only the ones that apply.
...
- `/launch-agent <folder> <task prompt>` starts a background agent with everything already in place: the topic folder exists, the launch prompt is saved inside it, the job carries a name you can find it by, and the agent points its own working files at that folder as its first action. Doing this by hand took four steps, and the ones that got forgotten were found out later, as a job named after nothing or a launch prompt that scrolled away. Name the folder with a slug to put it at your repository root, or with an absolute path to put it anywhere. Add `--here` when the work is yours to do now. It sets the folder and the prompt up the same way and starts no agent, so the session you called from becomes the session pointed at the folder. Reach for it above all when you invoke the skill from a session you started for the purpose: the default then leaves two rows in `claude agents`, the session that ran the skill and the agent it dispatched, and only the second one holds the work. ([issues/269](https://github.com/couimet/my-claude-skills/issues/269))
- `/launch-agent` refuses the mistakes that are only visible later. A slug that nearly matches a folder you already have is refused by name, so `agent-launch-skill` does not quietly become a second topic beside `agent_launch_skill`. A prompt that looks like a file path and names no readable file is refused too, rather than launching an agent whose whole prompt is the typo. An absolute path is read as deliberate and skips the near-match refusal, which is how you get past a refusal you meant. It does not skip the prompt refusal, which judges the prompt rather than the folder. ([issues/269](https://github.com/couimet/my-claude-skills/issues/269))
...
- `/start-issue` and `/tackle-pr-comment` no longer fail on the step that creates their draft working file. Both asked the path helper for a working-file type it does not accept, so every run of either skill printed `target-path T100 error: invalid --type 'scratchpad'` before the draft was written. ([issues/252](https://github.com/couimet/my-claude-skills/issues/252))
...
- New `/release-article` skill gathers release context (changelog, README, prior articles, promotion registry) and drafts a dev.to article with promotion copy and a cross-repo registration handoff. ([issues/228](https://github.com/couimet/my-claude-skills/issues/228))
...
- New `/note` skill — lightweight alternative to `/scratchpad` for capturing findings, results, and analysis with zero foundation skill dependencies; uses timestamp-based filenames and inlines branch detection for issue-scoped organization without token overhead ([issues/117](https://github.com/couimet/my-claude-skills/issues/117))
